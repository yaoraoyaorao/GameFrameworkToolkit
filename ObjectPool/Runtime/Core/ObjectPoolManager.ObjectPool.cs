    ...
    public sealed partial class ObjectPoolManager : IObjectPoolManager
    {
        private sealed class ObjectPool<T> : ObjectPoolBase, IObjectPool<T> where T : ObjectBase
        {
            ...
            public void Register(T obj, bool spawned)
            {
                ...
                if (!m_Objects.TryGetValue(obj.Name,out LinkedList<Object<T>> list))
                {
                    list = new LinkedList<Object<T>>();
                   
                    m_Objects.Add(obj.Name, list);
                   
                }
                else
                {
                    list.AddLast(internalObject);
                }
            ...
            public bool CanSpawn(string name)
            {
                ...
                if (m_Objects.TryGetValue(name, out LinkedList<Object<T>> list))
                {
                    foreach (Object<T> internalObject in list)
                    {
            ...
            public void Unspawn(object target)
            {
                ...
                else
                {
                    string name = !string.IsNullOrEmpty(Name) ?
                                    ...
                                    typeof(T).FullName;
                    throw new Exception(string.Format("Can not find target in object pool '{0}', target type is '{1}', target value is '{2}'.", name, target.GetType().FullName, target));
                }
            }
            ...
            public bool ReleaseObject(object target)
            {
                ...
                m_Objects[internalObject.Name].Remove(internalObject);
